...
if (isset ($_POST['channel_id']) && $_POST['channel_id'] > 0) {
    $sql = "UPDATE table_channel 
			SET channel_name = :name,
			channel_description = :description
			WHERE channel_id=:id";
} else {
    $sql = "INSERT INTO table_channel (channel_name, channel_description, channel_creation_date)
...
if (isset ($_POST['channel_name'])) {
    $stmt->bindParam(':name', $_POST['channel_name']);
}

if (isset ($_POST['channel_description'])) {
    $stmt->bindParam(':description', $_POST['channel_description']);
}

$stmt->execute();

if (isset ($_FILES['channel_icon']) && $_FILES['channel_icon']['name'] != "") {

    // Vérifier s'il y a une erreur
    if ($_FILES['channel_icon']['error'] != 0) {
    ...
    $sql = "SELECT channel_icon FROM table_channel
    WHERE channel_id = :channel_id";
    $stmt = $db->prepare($sql);
    $stmt->bindValue(":channel_id", $_POST['channel_id'] > 0 ? $_POST['channel_id'] : $channel_id);
    $stmt->execute();

    if ($row = $stmt->fetch()) {
        if ($row['channel_icon'] != "" && file_exists($uploadPath . $row['channel_icon'])) {
            unlink($uploadPath . $row['channel_icon']);
        }
    }

    // Renomme l'image

    $extension = pathinfo($_FILES['channel_icon']['name'], PATHINFO_EXTENSION);
    $filename = generateFileName($_POST['channel_name'], $extension, $uploadPath);
    move_uploaded_file(
        $_FILES['channel_icon']['tmp_name'],
        $uploadPath . $filename . "." . $extension
    );

    // Requête pour ajouter l'image dans la BDD

    $sql = "UPDATE table_channel SET channel_icon=:channel_icon 
    WHERE channel_id = :channel_id";

    $stmt = $db->prepare($sql);
    $stmt->bindValue(":channel_icon", $filename . "." . $extension); // Value prend la valeur là où on la déclare Param prend en compte les modifications
    $stmt->bindValue(":channel_id", $_POST['channel_id'] > 0 ? $_POST['channel_id'] : $channel_id);
    $stmt->execute();
    ...
    foreach ($tabTailles as $taille) {
        ...
        switch (strtolower($extension)) {
            ...
            default:
                ...
                $sql = "UPDATE table_channel SET channel_icon=null
                WHERE channel_id = :channel_id";
